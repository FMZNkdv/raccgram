...
public class RaccgramHiddenKeywordsActivity extends BaseFragment {
    ...
    private void reloadRows() {
        keywords.clear();
        keywords.addAll(RaccgramConfig.getInstance().getHiddenKeywords());

        rowCount = 0;
        headerRow = rowCount++;
        firstKeywordRow = rowCount;
        rowCount += keywords.size();
        addRow = rowCount++;
        sectionRow = rowCount++;
    }
    ...
    public View createView(Context context) {
        ...
        actionBar.setTitle("Hidden Keywords");
        ...
        listView.setOnItemClickListener((view, position) -> {
            if (position == addRow) {
                showAddDialog();
            } else if (position >= firstKeywordRow && position < firstKeywordRow + keywords.size()) {
                showDeleteMenu(view, keywords.get(position - firstKeywordRow));
            }
        });
    ...
    private void showAddDialog() {
        ...
        editText.setHintText("Word or phrase");
        ...
        AlertDialog dialog = new AlertDialog.Builder(getParentActivity())
                ...
                .setPositiveButton("Add", (d, w) -> {
                    ...
                    if (!TextUtils.isEmpty(text)) {
                        RaccgramConfig.getInstance().addHiddenKeyword(text);
                        reloadRows();
                        listAdapter.notifyDataSetChanged();
    ...
    private class ListAdapter extends RecyclerListView.SelectionAdapter {
        ...
        public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
            if (position == headerRow) {
                ((HeaderCell) holder.itemView).setText("Messages containing these words will be hidden");
            } else if (position == addRow) {
                TextSettingsCell cell = (TextSettingsCell) holder.itemView;
                cell.setTextColor(Theme.getColor(Theme.key_windowBackgroundWhiteBlueText4));
                cell.setText("Add Keyword", false);
            } else if (position >= firstKeywordRow && position < firstKeywordRow + keywords.size()) {
                TextSettingsCell cell = (TextSettingsCell) holder.itemView;
                cell.setTextColor(Theme.getColor(Theme.key_windowBackgroundWhiteBlackText));
                cell.setText(keywords.get(position - firstKeywordRow), true);
            }
        }
