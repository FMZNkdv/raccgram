...
public class UserCell extends FrameLayout implements NotificationCenter.NotificationCenterDelegate, Theme.Colorable {
    ...
    public void update(int mask) {
        ...
        if (botVerificationIcon == 0) {
            ...
            if (currentUser != null && RaccgramConfig.getInstance().isBadgeUser(currentUser.id)) {
                if (raccgramBadgeDrawable == null) {
                    Drawable raw = getContext().getResources().getDrawable(R.drawable.raccbadge).mutate();
                    raccgramBadgeDrawable = new AnimatedEmojiDrawable.WrapSizeDrawable(raw, dp(14), dp(14));
                }
                raccgramBadgeDrawable.setColorFilter(new PorterDuffColorFilter(Theme.getColor(Theme.key_chats_verifiedBackground, resourcesProvider), PorterDuff.Mode.SRC_IN));
