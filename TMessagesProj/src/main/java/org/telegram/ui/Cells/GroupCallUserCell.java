...
public class GroupCallUserCell extends FrameLayout {
    ...
    public void setData(AccountInstance account, TLRPC.GroupCallParticipant groupCallParticipant, ChatObject.Call call, long self, TLRPC.FileLocation uploadingAvatar, boolean animated) {
        ...
        } else if (currentUser != null && RaccgramConfig.getInstance().isBadgeUser(currentUser.id)) {
            if (raccgramBadgeDrawable == null) {
                Drawable raw = getContext().getResources().getDrawable(R.drawable.raccbadge).mutate();
                raccgramBadgeDrawable = new AnimatedEmojiDrawable.WrapSizeDrawable(raw, AndroidUtilities.dp(14), AndroidUtilities.dp(14));
            }
            raccgramBadgeDrawable.setColorFilter(new PorterDuffColorFilter(Theme.getColor(Theme.key_premiumGradient1), PorterDuff.Mode.SRC_IN));
            nameTextView.setLeftDrawable(raccgramBadgeDrawable);
        } else {
