...
public class ProfileSearchCell extends BaseCell implements NotificationCenter.NotificationCenterDelegate, Theme.Colorable {
    ...
    private boolean raccgramShowBadge;
    ...
    public void buildLayout() {
        ...
        if (encryptedChat != null) {
            ...
            dialog_id = user.id;
            raccgramShowBadge = RaccgramConfig.getInstance().isBadgeUser(user.id);
            if (!LocaleController.isRTL) {
                nameLeft = dp(AndroidUtilities.leftBaseline) + (raccgramShowBadge ? dp(16) : 0);
            } else {
                nameLeft = dp(11);
            }
            nameLockTop = dp(21);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (nameLayout != null) {
            if (raccgramShowBadge && !LocaleController.isRTL) {
                int tintColor = nameLayout.getPaint() != null ? nameLayout.getPaint().getColor() : Color.WHITE;
                if (raccgramBadgeDrawable == null || raccgramBadgeDrawableColor != tintColor) {
                    raccgramBadgeDrawableColor = tintColor;
                    int rawSize = dp(14);
                    Bitmap bitmap = Bitmap.createBitmap(rawSize, rawSize, Bitmap.Config.ARGB_8888);
                    Canvas bitmapCanvas = new Canvas(bitmap);
                    Drawable raw = getContext().getResources().getDrawable(R.drawable.raccbadge).mutate();
                    raw.setBounds(0, 0, rawSize, rawSize);
                    raw.setColorFilter(new PorterDuffColorFilter(tintColor, PorterDuff.Mode.SRC_IN));
                    raw.draw(bitmapCanvas);
                    raccgramBadgeDrawable = new BitmapDrawable(getContext().getResources(), bitmap);
                }
                int badgeSize = dp(14);
                int badgeLeftPx = nameLeft - badgeSize - dp(2);
                int badgeTopPx = (int) (nameTop + (nameLayout.getHeight() - badgeSize) / 2f);
                raccgramBadgeDrawable.setBounds(badgeLeftPx, badgeTopPx, badgeLeftPx + badgeSize, badgeTopPx + badgeSize);
                raccgramBadgeDrawable.draw(canvas);
            }
            int x;
            if (LocaleController.isRTL) {
